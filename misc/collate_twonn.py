...
for ds in dses:
    ...
    for m in models:
        cur_res[m] = res[ds][m]
    plot_per_ds_across_models(m_norm_layer_idxs, ds, cur_res, prop = False)
    plot_per_ds_across_models(m_norm_layer_idxs, ds, cur_res, prop = True)
...
def plot_per_model_per_ds_per_layeridx(cur_x, cur_ds, cur_li, model_size, id_arr, prop = False):
    ...
    fig, ax = plt.subplots(figsize=(7, 5))
    if prop == False:
        ax.plot(cur_x, id_arr)
    else:
        ax.plot(cur_x, id_arr/ffn_dim)
    ax.set_xlabel(cur_xlabel)
    ax.set_ylabel(ylab)
    ax.set_title(cur_title)
    cur_loc = 'center right'
    #ax.legend(loc=cur_loc, bbox_to_anchor=(1,0.5))
    plt.tight_layout()
    res_dir = UMN.by_projpath_multi(subpaths=[GRAPH_FOLDER, subf, cur_ds, model_size], make_dir = True)
    fpath = os.path.join(res_dir, fname)
    plt.savefig(fpath)
    fig.clear()
    plt.close()
