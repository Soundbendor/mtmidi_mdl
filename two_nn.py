...
def get_dist_idxs(cur_idx, N):
    idxs = torch.arange(N)
    ipts = torch.vstack((torch.ones(N-1, dtype=torch.int64) * cur_idx, torch.hstack((idxs[:cur_idx], idxs[(cur_idx+1):])))).sort(axis=0,descending=False).values
...
def get_mu_i(cur_idx, N, dist_vec,device='cpu'):
    ...
    idxs = get_dist_idxs(cur_idx, N)
    cur_dists = dist_vec[idxs]
